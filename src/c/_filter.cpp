...
std::tuple<bool, char> filterBothStrandAboveAverageMerge(std::map<char, int> & basecountNotCombined,
                                       std::map<char, int> & basecountExtendedFrags)
{
    ...
    return {bothObserved, aboveAverage};
}
...
std::tuple<double, int, int> finalEstimationWithJenks(std::vector<double> VAFList){
    ...
    int maxIterLim = 10;
    int minIterLim = 3;
    int maxIterXLim = VAFList.size()/20;
    ...
    for( int ct = minIterLim; ct < maxLim; ct++){
        auto [groups, breaks] = jenks(VAFList, ct);
    ...
    return {sumVector(include)/include.size(), include.size(), n};
}