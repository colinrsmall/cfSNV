...
if X_query.shape[0] == 0:
	open(overlap_pass + ".RFpred.csv", 'w').close()
else:
	y_query_predict=clf_overlap.predict_proba(X_query)[:,1]
	save_filename = overlap_pass + ".RFpred.csv"
	np.savetxt(save_path+save_filename, y_query_predict, delimiter=",")
...
if X_query.shape[0] == 0:
	open(nonoverlap_pass + ".RFpred.csv", 'w').close()
else:
	y_query_predict=clf_nonoverlap.predict_proba(X_query)[:,1]
	save_filename = nonoverlap_pass + ".RFpred.csv"
	np.savetxt(save_path+save_filename, y_query_predict, delimiter=",")
